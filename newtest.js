...
$(document).ready(function () {
    ...
    kintone.api(kintone.api.url("/k/v1/records", true), "GET", body, function (
        resp
    ) {
        $("#example-post").html("");
        ...
        $("#example-post").multiselect({
            ...
            templates: {
                ...
                filterClearBtn:
                    '<button class="btn btn-default multiselect-clear-filter" type="button"><i class="fas fa-times"></i></button>',
                li:
            ...
            resp.records.forEach((item) => {
                ...
                kintone.api(
                    ...
                    function (resp) {
                        ...
                        kintone.api(
                            ...
                            function (resp) {
                                if (
                                    resp.records[0].sessionworkid.value == "1"
                                ) {
                                    totalOffTime += 8 * cout;
                ...
                var body = {
                    ...
                    query:
                        ...
                        '"  order by date asc limit 100 offset 0',
                    fields: [
                        "$id",
                        "worktime",
                        "overtime",
                    ...
                    function (resp) {
                        ...
                        if (resp.records.length > 0) {
                            ...
                            resp.records.forEach((i) => {
                                ...
                                var lwt = strwt.split(":");
                                ttWorkTime +=
                                    parseInt(lwt[0]) + parseInt(lwt[1]) / 60;
                                workItem =
                                    parseInt(lwt[0]) + parseInt(lwt[1]) / 60;
                            ...
                            tr.append(
                                $('<td class="wt">' + ttWorkTime + "</td>")
                            );
                            tr.append(
                                $('<td class="ot">' + ttOverTime + "</td>")
                            );
...
async function viewDetail(chatworkid, dataStr, wt, ot, offtime) {
    ...
    kintone.api(
        ...
        function (resp) {
            // success
            $("#title").html(
                resp.records[0].department.value +
                    " -<span class='stn'> " +
                    resp.records[0].staffname.value +
                    "</span> (Chatwork ID︰ " +
                    chatworkid +
                    ")" +
                    " - [" +
                    resp.records[0].starttime.value +
                    " ~ " +
                    resp.records[0].endtime.value +
                    "]"
            );
        },
        function (error) {
        ...
        for (var daye = start; daye <= end; daye.setDate(daye.getDate() + 1)) {
            ...
            if (listHoliday.length > 0) {
                listHoliday.forEach((item) => {
                    ...
                    if (item.date.value == datec) {
                        tr.append(
                            $(
                                '<td class="first holiday">' +
                                    getFm(daye.getMonth() + 1) +
                                    "/" +
                                    getFm(daye.getDate()) +
                                    " (" +
                                    thu[daye.getDay()] +
            ...
            if (obj !== null) {
                ...
                if (obj.reason != null) {
                    ...
                    tr.append(
                        $('<td colspan ="5">' + obj.reason.value + "</td>")
                    );
                    ...
                    tr.append(
                        $('<td style="display:none">' + obj.$id.value + "</td>")
                    );
                } // làm việc bth
                else {
                    var otHour = 0;
        ...
        $("td i.fa-pencil")
            ...
            .on("click", function () {
                ...
                if ($tr.find("td:nth-child(4)").text() == "") {
                    ...
                    var $reasonInput = $(
                        '<textarea class="editText" style="width:600px"></textarea>'
                    );
...
function getAllData() {
    dataAttendanceManagement = getData(listAppId.AttendanceManagement);
    dataHolidayManagement = getData(listAppId.HolidayManagement);
    dataDepartmentMaster = getData(listAppId.DepartmentMaster);
    dataHolidayMaster = getData(listAppId.HolidayMaster);
    dataShiftMaster = getData(listAppId.ShiftMaster);
    dataStaffMaster = getData(listAppId.StaffMaster);
    console.log('dt',dataAttendanceManagement);
}
async function getData(appId) {
    data = [];
    await fetch(null, null, appId).then(function (records) {
        data = records;     
    }).then( function (){
        return data;  
    });
...
function  fetch(opt_offset, opt_records, appId) {
    ...
    return kintone.api("/k/v1/records", "GET", params).then(function (resp) {
        ...
        if (resp.records.length === 500) {
            return fetch(offset + 500, records, appId);
        }
        return records;
    });
}
